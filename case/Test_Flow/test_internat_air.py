...
def paying_types():
    ...
    付款方式
    '''
    paying_params = basic_data.paying_types(mode='TPM_AIR', transnationalShipment=True)
...
class TestInternatSea(unittest.TestCase):
    ...
    def test_paying_types(self,paying_id):
        self._testMethodDoc = '国际空运遍历付款方式'
        id = None
        phone = self.mock_data.random_create_mobile_phone()
        try:
            result = self.internat_air.internat_air_save(**{'shippingInfo.paymentTypeCode': paying_id,
