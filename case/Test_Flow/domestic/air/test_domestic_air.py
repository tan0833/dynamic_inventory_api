...
from api_pack.api_business.domestic_air_api import DomesticAirApi
...
#付款方式
paying_types = basic_data.paying_types(mode='TPM_AIR', transnationalShipment=False)
...
#货物明细包装单位
line_package_unit_types = basic_data.line_package_unit_types(mode='TPM_AIR', transnationalShipment=False)
line_package_unit_types_list = jsonpath.jsonpath(line_package_unit_types,'$..id')


#货币类型
currency_type = basic_data.currency_types(mode='TPM_AIR', transnationalShipment=False)
currency_type_list = jsonpath.jsonpath(currency_type,'$..code')


#服务类型
server_level = basic_data.server_level(mode='TPM_AIR', transnationalShipment=False,loadingType='CTM_FTL')
...
class TestInternatSea(unittest.TestCase):
    ...
    @classmethod
    def setUp(cls):
        cls.domestic_air = DomesticAirApi(temp_dict)
        cls.log = Log()
    ...
    def test_paying_type(self, paying_type_id):
        ...
        try:
            result = self.domestic_air.domestic_air_save(**{'shippingInfo.paymentTypeCode': paying_type_id,
                                                            'referenceOrders.0.referenceOrderNo': phone})
            id = jsonpath.jsonpath(result, '$..data')[0]
            self.assertEqual(result.get('success'), True)

            res = self.domestic_air.domestic_air_submit(id)
    ...
    def test_server_level(self, server_level_id):
        ...
        phone = self.mock_data.random_create_mobile_phone()
        try:
            result = self.domestic_air.domestic_air_save(
