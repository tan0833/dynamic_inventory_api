...
from api_pack.api_business.internation_express_api import InternatExpressApi
...
#付款方式
paying_types = basic_data.paying_types(mode='TPM_EXPRESS', transnationalShipment=True)
...
#货物明细包装单位
line_package_unit_types = basic_data.line_package_unit_types(mode='TPM_EXPRESS', transnationalShipment=True)
line_package_unit_types_list = jsonpath.jsonpath(line_package_unit_types,'$..id')


#货币类型
currency_type = basic_data.currency_types(mode='TPM_EXPRESS', transnationalShipment=True)
currency_type_list = jsonpath.jsonpath(currency_type,'$..code')


#服务类型
server_level = basic_data.server_level(mode='TPM_EXPRESS', transnationalShipment=True,loadingType='CTM_FTL')
...
class TestInternatSea(unittest.TestCase):
    ...
    @classmethod
    def setUp(cls):
        cls.internat_express = InternatExpressApi(temp_dict)
        cls.log = Log()
    ...
    def test_paying_type(self, paying_type_id):
        ...
        try:
            result = self.internat_express.internat_express_save(**{'shippingInfo.paymentTypeCode': paying_type_id,
                                                            'referenceOrders.0.referenceOrderNo': phone})
            id = jsonpath.jsonpath(result, '$..data')[0]
            self.assertEqual(result.get('success'), True)

            res = self.internat_express.internat_express_submit(id)
            self.assertEqual(res.get('success'), True)
        except Exception as e:
            raise e
        finally:
            self.log.warning('国际快递id:%s,配载方式单位为：%s,参考单号：%s' % (id, paying_type_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
    ...
    def test_server_level(self, server_level_id):
        ...
        phone = self.mock_data.random_create_mobile_phone()
        try:
            result = self.internat_express.internat_express_save(
            ...
            self.log.warning('国际快递id:%s,服务类型：%s,参考单号：%s' % (id, server_level_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
    ...
    def test_cargo_type(self, cargo_type_id):
        ...
        try:
            ...
            self.log.warning('国际快递id:%s,货物类型：%s,参考单号：%s' % (id, cargo_type_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
    ...
    def test_package_unit_types(self, package_unit_type_id):
        ...
        try:
            ...
            self.log.warning('国际快递id:%s,包装单位：%s，参考单号：%s' % (id, package_unit_type_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
    ...
    def test_line_package_unit_types(self, line_package_unit_types_id):
        ...
        try:
            ...
            self.log.warning('国际快递id:%s,货物明细包装单位：%s，参考单号：%s' % (id, line_package_unit_types_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
    ...
    def test_currency_type_list(self, currency_type_list_id):
        ...
        try:
            ...
            self.log.warning('国际快递id:%s,货币：%s，参考单号：%s' % (id, currency_type_list_id, phone))
            temp_list.append(id)
        self.log.info('\n\n')
