...
def calc_api_test_coverage(swagger_api_count, api_test_count):
    ...
    logging.info(f'swagger接口总数：{swagger_api_count}')
    logging.info(f'测试覆盖的接口总数：{api_test_count}')  # 根据自己情况看是否减掉登录的接口
    try:
        api_test_coverage = f'{(api_test_count / swagger_api_count) * 100:.2f}%'
        logging.info(f'接口测试覆盖率：{api_test_coverage}')
    except ZeroDivisionError as e:
        logging.exception(f'swagger接口总数为 0')
        raise e
    else:
