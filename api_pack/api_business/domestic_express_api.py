...
class DomesticExpressApi:
    ...
    def domestic_express_save(self,**kwargs):
        '''
        国内快递保存，输入的kwargs字典暂时只支持一个键
    ...
    def domestic_express_submit(self,id):
        '''
        提交下单
        :param id:
        :return:
        '''
        domestic_express_submit_dict = self.basic_dict.get('domestic_express_submit')
        path_url = domestic_express_submit_dict.get('url') + '%s'%id
        url = self.basic_url + path_url
        method = domestic_express_submit_dict.get('method')
        header = self.basic_dict.get('header')
...
if __name__ == '__main__':
    ...
    i = DomesticExpressApi(temp_dict)
    a = i.domestic_express_save(**{"shippingInfo.consignee.addressCode":"测试",'referenceOrders.0.referenceOrderNo':13134567978945})
    # a = i.domestic_express_submit(id=4522581089311580160)
    print(a)