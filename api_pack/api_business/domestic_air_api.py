...
class DomesticAirApi:
    ...
    def domestic_air_save(self,**kwargs):
        '''
        国内空运保存，输入的kwargs字典暂时只支持一个键
    ...
    def domestic_air_submit(self,id):
        '''
        提交下单
        :param id:
        :return:
        '''
        domestic_air_submit_dict = self.basic_dict.get('domestic_air_submit')
        path_url = domestic_air_submit_dict.get('url') + '%s'%id
        url = self.basic_url + path_url
        method = domestic_air_submit_dict.get('method')
        header = self.basic_dict.get('header')
...
if __name__ == '__main__':
    ...
    i = DomesticAirApi(temp_dict)
    a = i.domestic_air_save(**{"shippingInfo.consignee.addressCode":"测试",'referenceOrders.0.referenceOrderNo':13134567978945})
    # a = i.domestic_air_submit(id=4522581089311580160)
    print(a)